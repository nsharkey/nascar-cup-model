#!/usr/bin/env python3
"""Parse raw feeds -> per-race driver feature table (pickle). My own code, not the prior session's."""
import json, os, glob, pickle, statistics as st
import numpy as np
    ...
    for r in d['series_1']:
        if r.get('race_type_id') == 1:
            races.append((r['race_date'], yr, r['race_id'], r['track_name'].strip(),
                          r.get('scheduled_laps')))
races.sort()

out = []
skipped = []
for dt, yr, rid, track, sched in races:
    ...
    if not lt or not wf or not wf.get('weekend_race'):
        skipped.append((dt[:10], track, 'no feed')); continue
    wr = wf['weekend_race'][0]
    results = wr.get('results') or []
    if len(results) < 20:
        skipped.append((dt[:10], track, f'results n={len(results)}')); continue
    ...
    if len(res) < 20:
        skipped.append((dt[:10], track, 'few valid finishers')); continue
    ...
    if len(med) < 30:
        skipped.append((dt[:10], track, f'green laps with field={len(med)}')); continue
    ...
    out.append(dict(date=dt, year=yr, rid=rid, track=track, drivers=table,
                    n_green=len(med), n_fe=len(fe), n_prac=len(prac)))

pickle.dump(out, open('races_parsed.pkl', 'wb'))
print(f'parsed {len(out)} races; skipped {len(skipped)}:')
for s in skipped: print('  ', s)
c2225 = sum(1 for r in out if r['year'] <= 2025)
print(f'\n2022-2025 completed races parsed: {c2225} (brief claims ~144)')
print(f'2026 races parsed (true out-of-sample): {len(out) - c2225}')
prac_cov = sum(1 for r in out if r['n_prac'] >= 20)
fe_cov = sum(1 for r in out if r['n_fe'] >= 15)
print(f'races with practice data: {prac_cov}/{len(out)}; with FE pace: {fe_cov}/{len(out)}')
