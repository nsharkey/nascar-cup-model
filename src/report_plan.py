...
def render_html(plan):
    ...
    for ph in plan["phases"]:
        ...
        for s in _sessions_of(plan, ph["key"]):
            ...
            trc = ' class="next"' if s["status"] == "next" else ""
            rows.append(
                f'<tr{trc}>'
 ...
 *{{box-sizing:border-box;}} body{{margin:0;}}
 ...
 .wall{{ font-size:.8rem; color:var(--grey); white-space:nowrap; font-variant-numeric:tabular-nums;}}
 ...
 .st.done{{color:var(--faint);}} .st.blocked{{color:var(--accent);}} .st.pending{{color:var(--grey);}} .st.next{{color:var(--accent);}} .st.half_done{{color:var(--grey);}}
 tr.next td{{ box-shadow:inset 4px 0 0 var(--accent); background:var(--panel);}} tr.next .st.next::before{{content:"■ ";}}
