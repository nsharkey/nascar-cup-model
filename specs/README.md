...
| `track_similarity.md` | F4 empirical track similarity vs structural edges (added 2026-07-20, plan session F4): Driver Skill Transferability (DST) — frozen-engine residual/expected-rank, leave-one-season-out CV weighted-Pearson pairwise estimator, `track_id`-only grain (deviates from F3), family-pair pooling reusing F3's `blend_values`; edge-restricted Spearman/top-3-Jaccard/named-disagreement-list comparison vs `silver.track_similarity_prior`; from-scratch pltree cross-validation of MY_TYPE (`external_knowledge_scan.md` §3.9) | Runs in F4 itself (analytics/reference tier — no gate, no A/B) |
| `loop_metric_histories.md` | F13 driver loop-metric histories (added 2026-07-20, plan session F13): in-house loop data derived from `silver.laps`/`silver.lap_flags` (C2) — ARP (pinned green-flag-only, one of three circulating public definitions), green-flag pass differential, quality passes, fastest-lap share, laps-in-top-15, closers; AS-OF (strictly-prior) histories only, half-life reused from the frozen config; green-flag-lap/pass imported verbatim from F3; a self-built composite supersedes NASCAR's own (circular) Driver Rating | Runs in F13 itself (analytics/reference tier — no gate, no A/B) |
